  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent
        ...
        className="w-[700px] max-w-[95vw] h-[625px] p-0 bg-white border border-[#E3E3E3] rounded-xl flex flex-col "
      >
        {loading ? (
          ...
          <>
            <div className="flex items-center justify-between px-10 py-5 border-b">
              <h2 className="text-2xl font-medium">Change Subscription</h2>
            ...
            <div className="flex flex-col gap-4 px-10 py-4 flex-1 overflow-y-auto">
              <div className="flex items-center justify-between bg-[#F7F7F7] p-4 rounded-lg">
                ...
                <p className="font-semibold">
                  ${currentPlan?.plan?.metadata?.amount}
                </p>

                <button
                  onClick={() => onCancelSubscription(currentPlan?._id)}
                  className="border cursor-pointer px-4 py-2 rounded-full text-red-600 text-sm"
                >
                  Cancel Subscription
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {plans
                  ?.filter((subs: any) => subs.type !== "buyer")
                  ?.map((subs: any, index: number) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`p-6 rounded-2xl border border-gray-200 hover:shadow-lg transition ${isExecutive ? "relative" : ""}`}
                      >
                        {isExecutive && (
                        ...
                        <div className="flex items-center gap-3 mt-4">
                          ...
                          <p className="text-sm text-gray-600 capitalize">
                            {" "}
                            {subs?.metadata?.watches ||
                              subs?.metadata?.bids}{" "}
                            watch / per {subs?.metadata?.interval}
                          </p>
                        </div>
                        <Button
                          disabled={
