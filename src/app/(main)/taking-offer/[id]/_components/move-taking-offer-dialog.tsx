...
import { useMoveToTakingOffers } from "@/features/products/hook";
type Props = {
...
const MoveToTakingDialog = ({
  ...
  const { mutateAsync: moveToTakingOfferMutate, isPending } = useMoveToTakingOffers();

const handleMoveToTakingOffer = async () => {
  const res = await moveToTakingOfferMutate({id});
  if (res?.success) {
          ...
          <div className="w-full">
            <div className="flex w-full justify-center gap-4">
              ...
              <Link href={"/"} className="w-[50%]">
                <Button
                disabled={isPending}
                  onClick={handleMoveToTakingOffer}
